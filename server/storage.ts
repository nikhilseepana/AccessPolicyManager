...
// Define interfaces for the schema with tables and fields
interface TableWithFields extends Table {
  fields: Field[];
}

interface SchemaWithTablesAndFields extends Schema {
...
export interface IStorage {
  ...
  getSchemaWithTablesAndFields(id: number): Promise<SchemaWithTablesAndFields | undefined>;
...
export class MemStorage implements IStorage {
    ...
    for (const schemaData of schemas) {
      ...
      if (schema.name === 'HR' && managerUser) {
        ...
        if (employeesTable) {
          ...
          const fieldNames = fields.filter((f) => f.name !== 'salary').map((f) => f.name);
  ...
  async createSchema(schemaData: SchemaUpload): Promise<Schema> {
    ...
    const schema: Schema = {
      id: schemaId,
      name: schemaData.name,
      description: schemaData.description, // Add the description field
      createdAt: now,
      updatedAt: now,
    };
    ...
    for (const tableData of schemaData.tables) {
      const table = await this.createTable({
        name: tableData.name,
        description: tableData.description, // Include table description
        schemaId: schemaId,
      });
      ...
      for (const fieldData of tableData.fields) {
        await this.createField({
          ...
          description: fieldData.description, // Include field description
          tableId: table.id,
        });
      }
    }
  ...
  async getSchemaWithTablesAndFields(id: number): Promise<SchemaWithTablesAndFields | undefined> {
  ...
  async getUserAccessRequests(userId: number): Promise<AccessRequest[]> {
    return Array.from(this.accessRequests.values()).filter((request) => request.userId === userId);
  }
  ...
  async getAccessRequestItems(requestId: number): Promise<AccessRequestItem[]> {
    return Array.from(this.accessRequestItems.values()).filter(
  ...
  async createAccessPolicy(policyData: InsertAccessPolicy): Promise<AccessPolicy> {
    ...
    const fields = policyData.fields
      ? Array.isArray(policyData.fields)
        ? policyData.fields
        : Array.from(policyData.fields as any)
      : null;
  ...
  async getUserAccessPolicies(userId: number): Promise<AccessPolicy[]> {
    return Array.from(this.accessPolicies.values()).filter((policy) => policy.userId === userId);
  }
  ...
  async checkPolicyConflict(
    ...
    const tablePolicies = userPolicies.filter((policy) => policy.tableId === tableId);
    ...
    for (const policy of tablePolicies) {
      ...
      if (effect !== policy.effect) {
        ...
        const overlappingFields = fields.filter((field) => policyFields.includes(field));
